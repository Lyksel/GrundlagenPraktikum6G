...
\begin{axis} [
    ...
    declare function={
        T=2;
        omegaNull=4*pi;
        sinc(\x) = (abs(\x)<0.0001) ? 1 : sin(deg(\x))/(\x);
    },
]
\addplot[blue, thick] {abs(T/2 * (sinc((x-omegaNull)*T/2) - sinc((x+omegaNull)*T/2)))};
\end{axis}
\end{tikzpicture}
\caption{Betragsspektrum von $\left| \mathcal{F}\{z(t)\} \right|$ für $T=2$, $\omega_0=4\pi$}
